...
def test_progression_engine_suggests_advancement(runner, setup_temp_data_path):
    ...
    runner.invoke(
        cli, ["micro", "add", "Walk 5 min", "--goal", "Fitness", "--phase", "Phase 1"]
    )
    runner.invoke(
        cli, ["micro", "add", "Walk 10 min", "--goal", "Fitness", "--phase", "Phase 1"]
    )
    runner.invoke(
        cli, ["micro", "add", "Walk 15 min", "--goal", "Fitness", "--phase", "Phase 1"]
    )
    ...
    with open(data_path, "r+") as f:
        ...
        micro_goal = goal["phases"][0]["micro_goals"][
            0
        ]  # Assuming 'Walk 5 min' is the first micro-goal
    ...
    assert (
        "Walk 5 min" in result.output
    )  # Assert that the current micro-habit is still active
        ...
        for j in range(5):  # 5 micro-habits per goal
            ...
            for k in range(90):  # 90 days of check-ins
                ...
                checkin_instance = Checkin(
                    date=checkin_date.date(), success=status, note=""
                )
                micro.checkins.append(checkin_instance)
            phase.micro_goals.append(micro)
        goal.phases.append(phase)
        goals_data.append(
            json.loads(goal.model_dump_json())
        )  # Use model_dump_json and json.loads

    with open(data_path, "w") as f:
        json.dump(goals_data, f, indent=2)

    # Action: Run report --mode calendar, report --mode success,
    # and report --mode line.
    # Assertion: All commands complete successfully without errors and
